...
=head1 SYNOPSIS
  ...
  # or pass in the argv needed to the main entry point in your own script
  #
  exit(App::TestOnTap::main(@ARGV));
  
=head1 OVERVIEW

B<NOTE: THIS IS CURRENTLY VIEWED TO BE IN BETA STATE. IT MAY CHANGE IN INCOMPATIBLE WAYS BEFORE THE 1.000 RELEASE.>
...
The core mechanism is to use TAP (e.g. "Test Anything Protocol") as the coupling between test executables and the application as
the test driver, e.g. very similar to when writing regular Perl module tests. To implement this, the standard "Test::*" and "TAP::*"
families of test modules are used, but with the necessary packaging to provide for the other requirements. Since the 
interface is 'TAP', any language can be used to write the tests.
...
named with an extension defined in the default "exec mapping" (see more about defaults in L<"Executable mappings">).
...
A test suite is configured by putting the file F<config.testontap> in the root of the suite.
...
If not present, it effectively defaults to include all files, assuming they match given extensions in the
"exec mapping" (see below).
...
As the suite is scanned for tests, paths will be tested against the query, and if not selected, will be pruned
completely from the set of possible tests. In particular, not selecting a directory will prune the entire subtree.

Example:

  include = not eq(unfinished_test.pl) and not regexp{^(.*/)?_data/$}
...
The value is a Grep::Query expression, and can be multi-line/part for readability.
...
surprises if other extensions are added to the default, and also to use the "include" key to ensure only expected files are
selected as tests regardless.

=item B<Section: [DEPENDENCY <name>]>
...
it will be suffixed with '.zip'. See L<Args|App::TestOnTap::Args> for details on how to control this using '--archive'
and '--savedirectory'.
