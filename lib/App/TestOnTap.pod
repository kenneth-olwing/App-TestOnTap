...
This section is technically optional, but recommended. It details how to decide which files are tests and how
they should be executed. Note that there is also possible to pass a file with this section manually using the
'--execmap' option, in order to override or complement any defined execmap.
...
If no "EXECMAP" section is present, an internal one can be used by specifying a commandline of
C<--execmap :internal> which uses an execmap equivalent to this (for this release, may change):

  [EXECMAP]
  # Perl, including common test extension
  match1 = regexp[\.(t|pl)$]
  cmd1 = perl
  # Python
  match2 = regexp[\.py$]
  cmd2 = python
  # Java (jar files with main entry point)
  match3 = regexp[\.jar$]
  cmd3 = java
       = -jar
  # Groovy with common extensions
  ...
  # only on windows
    ...
    cmd5 = cmd.exe
         = /c
    # AutoIt scripts (https://www.autoitscript.com/site/autoit/)
    match6 = regexp(\.au3)
    cmd6 = autoit3
  # only on unix/linux
    # Shell scripts
