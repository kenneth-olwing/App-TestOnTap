...
can influence how to run the tests or their context. It will be run with the initial environment and be passed the users arguments
(if any).
...
desired values enclosed with begin/end lines. Here's an example for the environment:

  # BEGIN ENV
  SOMEVARIABLE=somedata
  # END ENV

Note that this uses C<ENV> as the begin/end marker. For the argument list, use C<ARGV>. In both cases it's one variable or argument
per line.
