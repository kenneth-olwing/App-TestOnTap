...
=head1 SYNOPSIS
 ...
 testontap
 	[ @argfile ] # I<see 'EXPANDING FILES IN ARGUMENT LIST' below for details> 
    [
        --usage | -?
            |
        --help | -h
...
This turns on a special mode where tests are run with no capture and parsing of
...
execmap that starts the test in debug mode enables them to run under a real debugger.
 ...
 Example execmap file 'debug.execmap':
...
and run with:
 ...
 testontap --no-harness --execmap debug.execmap some/path/to/suite
 
would start all perl tests under the Perl debugger.
