...
The example command line C<testontap --verbose @myargs> will be expanded by reading the F<myargs> file (in the current
...
However, note that the actual order of tests are primarily controlled by dependencies, which will also
produce the correct run order when parallelization is in use. Thus, the order strategy is only relevant
when there are multiple to choose from during evaluation of 'next available test(s)'.
...
For most purposes the order will likely be random, but for a guaranteed randomization, use the C<random> strategy.
...
This turns on the display of timing information for tests when running.
